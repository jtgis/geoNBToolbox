...
def downloadFile(url,downloadToFolder):
    ...
    try:
        ...
        downloadedFile = os.path.join(downloadToFolder,file_name)
...
def mostRecentLidar(inputFeatureClass):
    ...
    for fc in lidarMapServiceList:
        ...
        arcpy.management.AddField(unionedLidar,"DOWNLOAD","TEXT",field_length=150)
        tempList.remove(fc[0])
        query = """FID_{} >= 0""".format(fc[0])
        for i in tempList:
            query += """ AND FID_{} < 0""".format(i)
        arcpy.analysis.Select(unionedLidar,currFC,query)
        arcpy.CalculateField_management(currFC,"DOWNLOAD",lidarDownloadID,"VB")
...
class downloadLiDAR(object):
    ...
    def getParameterInfo(self):
        ...
        createLidarProducts = arcpy.Parameter(
                                      displayName="Create LiDAR Products: DEM, DSM, Slope,Aspect, Contours, Hillshade, and CHM",
    ...
    def execute(self, parameters, messages):
        ...
        if inputType == "PID":
            ...
            selectFeatures = downloadRestFeatures(pidMapService,"",query,"selectFeatures")
        else:
            selectFeatures = inputFeatureClass

        lidarFC = mostRecentLidar(selectFeatures)

        lidarFCLayer = arcpy.MakeFeatureLayer_management(lidarFC,"lidarFCLayer")

        arcpy.SelectLayerByLocation_management(lidarFCLayer,"INTERSECT",selectFeatures,"","NEW_SELECTION")
